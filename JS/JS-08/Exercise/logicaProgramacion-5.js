...
class Adivinador {
    ...
    adivinarNumero() {
        ...
        if (isNaN(num) || num === "") {
            ...
            this.adivinarNumero();
        } //isnan() devuelve true si el valor no es un número, false si lo es. Si el valor es un string vacío, isNaN() devuelve true.
        else {
            num = parseInt(num);
            if (num < 1 || num > 100) {
            ...
            }//La funcion del else es que si el numero introducido es correcto, se muestre el mensaje de felicidades y la lista de numeros introducidos. Si no es correcto, se vuelva a ejecutar la funcion adivinarNumero() para que vuelva a pedir un numero.
        }
    }
}
// Generar un número aleatorio del 1 al 100 como número secreto
let numSecreto = Math.floor(Math.random() * 100) //Math.floor() redondea hacia abajo, el número aleatorio generado por Math.random().
